...
try {
    ...
    New-Item -ItemType Junction -Path (Join-Path $stage 'node_modules/zod') -Target (Join-Path $dependencies 'zod') | Out-Null
    ...
    Push-Location $stage
    try { & (Join-Path $upstreamRoot 'node_modules/.bin/tsdown.cmd') --config tsdown.config.ts; if ($LASTEXITCODE -ne 0) { throw 'EduWork workbench client build failed' } }

...
const req=createRequire(join(runtime,'package.json')),ts=createRequire(join(root,'.cache/client-build-tools/package.json'))('typescript')
...
const server=createServer(async(request,response)=>{
 ...
 if(request.url.startsWith('/api/')) {
  ...
  if(action==='check-updates')state='available'
  ...
  response.setHeader('content-type','application/json');response.end(JSON.stringify({version:'0.3.5-dev.20260912.1',shell:'wails',phase:state,update:{state,policy,enabled:true,latestVersion:'0.3.5',downloadedBytes:downloaded,totalBytes:1048576,installOnNextStart:scheduled}}));return
 }
...
try {
 ...
 await pill.click();await page.getByLabel('更新下载进度').waitFor()
 ...
 const dialog=page.getByRole('dialog',{name:'软件更新'});await dialog.waitFor()

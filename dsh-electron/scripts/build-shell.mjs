...
for (const name of ['update-coordinator.mjs', 'portable-updates.mjs', 'native-vault.mjs', 'product.mjs', 'lifecycle.mjs', 'media-transport.mjs', 'configuration-files.mjs', 'configuration-policy.mjs', 'legacy-migration.mjs', 'external-navigation.mjs']) await copyFile(join(repository, 'dsh-electron/src', name), join(output, 'src', name))
...
for (const name of ['user-config.mjs', 'content-updates.mjs', 'content-update-protocol.mjs']) await copyFile(join(repository, 'dsh-host', name), join(output, 'src', name))

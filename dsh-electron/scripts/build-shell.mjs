...
for (const name of ['user-config.mjs', 'configuration-file.mjs', 'configuration-documentation.mjs', 'configuration-reference.mjs', 'content-updates.mjs', 'content-update-protocol.mjs', 'publisher-bootstrap.mjs']) await copyFile(join(repository, 'dsh-host', name), join(output, 'src', name))
...
for (const name of ['configuration-documentation.mjs', 'configuration-reference.mjs']) adapters['dsh-host/' + name] = digest(await readFile(join(repository, 'dsh-host', name)))

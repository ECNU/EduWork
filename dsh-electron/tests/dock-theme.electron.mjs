...
const images = ['icon-1024.png', 'icon-blue-1024.png']
...
app.whenReady().then(async () => {
  ...
  try {
    ...
    await select('dsh', 1, 'icon-blue-1024.png')
    await select('ecnu-liwa', 2, 'icon-1024.png')
    await select('dsh', 3, 'icon-blue-1024.png')
    ...
    await window.loadURL('dsh-app://app/index.html')
    await select('ecnu-liwa', 4, 'icon-1024.png')
    ...
    for (const [style, color] of [['dsh', 'rgb(37, 117, 255)'], ['ecnu-liwa', 'rgb(159, 38, 54)']]) {
      ...
      splash = await render(appShim(), BrowserWindow, process, join, readFileSync, savedEduworkStyle)
    ...
    function appShim() { return { getAppPath: () => appRoot, dock: app.dock } }
    ...
    console.log(`PASS: sandbox preload, red/blue/reload, trusted frame and style checks, persisted startup images and colors, unchanged icon resources, write failure (Electron ${process.versions.electron})`)

...
An explicit `--native-017` build uses the [DSH 0.1.7-rc.2 candidate lock](../third_party/dsh/candidate-v0.1.7-rc.2/LOCK.json): the core, Web client, Desktop Host and authenticated HTTP transport come from `477b4f420553e8a52c2fbccc464d7561b239c443`. The window baseline remains independently pinned; receipts record both revisions. Product adapters retain organization login, system-encrypted credentials, portable updates, task notifications and Studio. This is not a repackaged official desktop installer and does not inherit its signature. Preset selection follows rc.2's Coding Tools preference while retaining optional-mode toggles. See the [candidate CI](../.github/workflows/validate-core-017.yml) for build and runtime checks; this mode does not change the default release lock or update feeds.

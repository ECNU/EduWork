...
export function githubUpdateManifest(receipt,repository) {
 ...
 if(!identity || receipt.passed!==true || receipt.kind!=='eduwork-windows-release' || !/^(0|[1-9]\d*)\.(0|[1-9]\d*)\.(0|[1-9]\d*)$/.test(receipt.version) || receipt.edition!==identity[0] || receipt.distribution!==identity[1] || receipt.shell!=='electron' || receipt.platform!=='windows-x64')throw Error('Update manifest requires a matching validated stable Electron release')
 ...
 return {schemaVersion:1,distribution:identity[1],channel:'stable',version:receipt.version,target:'windows-amd64',artifacts:[{shell:'electron',flavor:'offline',fileName:asset.name,bytes:asset.bytes,sha256:asset.sha256,url,sha256Url:url+'.sha256'}]}
}

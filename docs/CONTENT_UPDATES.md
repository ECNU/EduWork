...
下载内容暂存在 `data/content-updates/`，不改动程序。下载失败可以重新检查后重试；内容包较小，重试重新下载。文件应用阶段与桌面试运行分别记录，仅在桌面完成加载后提交新的生效状态。启动失败或试运行被中断，配置恢复唯一备份、Skills 恢复上一生效修订；更新期间的手工配置修改保留。

权限、文件占用或磁盘空间等本地读写问题，以及在进入桌面试运行前中断的文件事务，不再把内容修订标记为坏版本。修正本机问题后可重试同一签名内容。真正启动失败的内容仍停止自动重试，发行方修改内容时须发布更高修订号。用户主动“检查更新”、在首次启动窗口“重试下载”或导入签名离线包，可重新验证并试用最高已接收修订中的同一失败内容包，也适用于旧版留下的失败记录；后台检查不授权这种重试。所有签名、渠道、依赖、组件版本及防倒退检查仍生效，不接受更低修订或同号不同包。

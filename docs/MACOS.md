...
应用从系统或用户的“应用程序”目录成功启动后，如果检测到唯一匹配的 DMG 安装卷，会提示“推出并移到废纸篓”或“保留”。匹配要求安装卷仅包含一个应用，且其签名校验通过、签名标识与当前应用一致；多个候选、从安装卷运行或无法确认来源时不提示。

确认清理后先正常推出安装卷，再将对应 DMG 移到废纸篓。卷被占用、文件发生变化或操作失败时提示手动处理，不强制推出。选择保留或成功清理后，同一应用签名不再重复提示。已安装的应用不受影响。

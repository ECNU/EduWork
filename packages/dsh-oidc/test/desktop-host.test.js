...
test(`DSH ${dshVersion} Host attaches optional browser lifecycle and performs HTTP PKCE without WebServer`, async t => {
  ...
  assert.equal(ctx.get('webServer'), undefined)
  const relogin = await ctx.oidcAccounts.begin(f.profile.id)
  const reauth = await fetch(f.opened.at(-1), { redirect: 'manual' })
  assert.equal((await fetch(reauth.headers.get('location'))).status, 200)
  assert.equal((await ctx.oidcAccounts.loginStatus(relogin.loginID)).state, 'completed')
  await ctx.oidcAccounts.status(f.profile.id)
  await ctx.oidcAccounts.status(f.profile.id)
  assert.deepEqual(events.map(value => value.state), ['connected', 'connected'], 'new authorization must notify even when its state name is unchanged; status polling must not')
  assert.equal(events[1].authorizationChanged, true)
  await ctx.oidcAccounts.logout(f.profile.id)
  assert.deepEqual([...f.records.keys()], ['PERSONAL_API_KEY'])
  assert.deepEqual(events.map(value => value.state), ['connected', 'connected', 'signed_out'])

...
For OAuth Access Token services, use `authorizedFetch(profileID, endpoint, init, authorization?)`. In `auth` mode, requests first refresh tokens with 90 seconds or less remaining; GET/HEAD also refresh and retry at most once after a 401. Trusted Host services may pass `{ retryUnauthorized: true }` as the fourth argument for safely repeatable POST requests such as activity heartbeats. The body must be a buffered string or absent, never a stream. This option stays out of HTTP requests and client RPC, and does not broaden the allowed origins or paths. Model generation does not opt in and is never automatically replayed; normal token rotation within the same authorization does not cancel active streams. See [the public protocol](public-resource-protocol.en.md). Neither transport schedules business requests itself.

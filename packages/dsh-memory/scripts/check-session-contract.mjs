...
import { explicitMemoryRequest, executionSources, memoryMessageSource } from '../lib/session-context.js'
...
const source = memoryMessageSource(session, 'synthetic fixture')
session.append('user/message', createUserMessage({ content: [{ type: 'text', text: 'Please remember this' }], source }), { surfaceOp: 'append' })
assert.equal(source.kind, session.header.version >= 4 ? 'plugin:memory-native' : 'plugin')
assert.equal(explicitMemoryRequest(session), false)
